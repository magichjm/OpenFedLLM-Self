...
# 这个函数统一所有数据集的格式为(instruction,response)
def process_sft_dataset(dataset_name, dataset, dataset_sample):
    # 输出原始数据量
    print(f">> Dataset `{dataset_name}` originally has {len(dataset)} examples.")
...
# 这个函数是把instruction和input合并，因为有的数据集是包含这两块的，统一一下
# 然后拼上response返回整条数据(instruction,response)
